...
describe('first-account wizard validation', () => {
  ...
  it('accepts a complete draft and rejects bad fields', () => {
    ...
    assert.match(validateOnboardingField('plate', '12'), /plate|sticker|short/i)
    assert.equal(validateOnboardingField('plate', '847291'), '')
    assert.equal(validateOnboardingField('plate', 'TMP 1234'), '')
