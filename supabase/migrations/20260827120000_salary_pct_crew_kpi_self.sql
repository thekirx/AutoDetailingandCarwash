-- Owner revisions P6: optional services.salary_pct + crew KPI self-serve for staff/detailer
begin;

alter table public.services
  add column if not exists salary_pct numeric;

alter table public.services
  drop constraint if exists services_salary_pct_range;

alter table public.services
  add constraint services_salary_pct_range
  check (salary_pct is null or (salary_pct >= 0 and salary_pct <= 100));

comment on column public.services.salary_pct is
  'Optional catalog salary % for floor preview only. Null = use global wash pool / ceramic splits.';
...
begin
  ...
  if caller_role not in (
    'admin', 'BossMich', 'team_lead', 'assistant_super_admin', 'operations_lead',
    'staff', 'detailer'
  ) then
    raise exception using errcode = '42501', message = 'Crew KPI access is restricted';
  end if;

  if caller_role in ('staff', 'detailer') then
    self_only := true;
  end if;
  ...
  with completed_sessions as (
    ...
    where qa.status = 'released'
      ...
      and (not self_only or qa.staff_id = caller_id)
  ...
  failed_qa_sessions as (
    ...
    where b.redo_at is not null
      ...
      and sid is not null
      and (effective_branch is null or b.branch = effective_branch)
      and (not self_only or sid = caller_id)
  ...
  where (not self_only or k.staff_id = caller_id)
