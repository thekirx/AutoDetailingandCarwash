-- Part 5: expense category kinds + write grants; POS allows assistant_super_admin
...
begin
  if not exists (
    ...
    where conname = 'expense_categories_kind_check'
  ) then
    alter table public.expense_categories
      add constraint expense_categories_kind_check
      check (kind in ('general', 'payroll', 'marketing', 'utilities', 'chemicals', 'equipment'));
  end if;
end $$;

update public.expense_categories set kind = 'payroll' where lower(name) = 'payroll';
update public.expense_categories set kind = 'marketing' where lower(name) = 'marketing';
update public.expense_categories set kind = 'utilities' where lower(name) = 'utilities';
update public.expense_categories set kind = 'chemicals' where lower(name) = 'chemicals' or is_chemical;
update public.expense_categories set kind = 'equipment' where lower(name) = 'equipment';

create index if not exists expense_categories_kind_idx on public.expense_categories (kind);
...
-- Keep live complete_pos_sale body; only widen roles for Part 2/5 RBAC
...
begin
  ...
  -- ponytail: BossMich + Assistant Super Admin can post any branch; Admin matches assigned branch
