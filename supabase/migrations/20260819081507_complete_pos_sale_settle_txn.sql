-- Reject orphan queue lines (sale_line_item_ref requires service_id).
-- Mark the handoff's pending_payment transaction completed when POS pays.
...
begin
  ...
  if v_handoff is not null and v_status = 'paid' then
    ...
    where t.status = 'pending_payment'
      and t.pos_handoff_id = v_handoff;
  end if;
