...
export default function LoyaltyCard({ visitCount = 0 }) {
  ...
  return (
    <section className="relative overflow-hidden rounded-[2rem] border border-line bg-[#0d1316] p-6 shadow-2xl sm:p-10">
      ...
      <div className="relative">
        <div className="mb-8 flex flex-col justify-between gap-5 sm:flex-row sm:items-start">
          <div>
            <div className="mb-4 flex items-center gap-2 text-[10px] font-extrabold uppercase tracking-[.28em] text-acid"><Sparkles size={14} /> Hakum rewards</div>
            <h3 className="max-w-xl font-display text-3xl uppercase leading-tight sm:text-4xl">{getEncouragement(completed)}</h3>
            <p className="mt-3 text-sm text-mist">Every visit brings your ride—and your next reward—closer.</p>
          </div>
          <div className="flex shrink-0 items-baseline gap-2 sm:block sm:text-right"><span className="font-display text-4xl text-acid">{completed}</span><span className="text-xs font-bold uppercase tracking-widest text-mist"> / {TOTAL_SLOTS} visits</span></div>
        </div>
        ...
        <div className="mb-8 h-1.5 overflow-hidden rounded-full bg-white/5" aria-label={`${progress}% loyalty progress`} role="progressbar" aria-valuemin="0" aria-valuemax="15" aria-valuenow={completed}>
          <div className="h-full rounded-full bg-acid shadow-[0_0_16px_rgba(233,255,63,.65)] transition-all duration-700" style={{ width: `${progress}%` }} />
        </div>
            ...
            return (
              <li key={number} className="grid justify-items-center gap-2">
                <div aria-label={stamped ? `Visit ${number} completed${reward ? `, ${reward}` : ''}` : `Visit ${number}${reward ? `, ${reward}` : ''}`} className={`stamp-slot relative grid aspect-square w-full max-w-[82px] place-items-center rounded-full border ${stamped ? 'stamp-slot-complete' : 'border-white/10 bg-black/20'} ${milestone ? `stamp-milestone stamp-milestone-${number}` : ''}`}>
                  {stamped ? <span className="stamp-logo grid h-[68%] w-[68%] -rotate-6 place-items-center rounded-full border-2 border-ink bg-acid font-display text-xl text-ink shadow-lg sm:text-2xl">H</span> : milestone ? <Gift size={20} className="text-acid/75" /> : <span className="text-xs font-bold text-white/20">{String(number).padStart(2, '0')}</span>}
                  {stamped && <span className="absolute inset-1 rounded-full border border-acid/30" />}
                </div>
                <span className={`text-[8px] font-extrabold uppercase tracking-[.15em] sm:text-[9px] ${milestone ? 'text-acid' : 'text-mist/60'}`}>{reward || `Visit ${number}`}</span>
              </li>
            )
          })}
        </ol>

        <div className="mt-8 flex flex-wrap items-center justify-between gap-3 border-t border-line pt-5 text-[10px] font-bold uppercase tracking-[.18em] text-mist">
          <span>10 visits · Free wash</span><span className="text-acid">15 visits · Premium detail</span>
        </div>
      </div>
    </section>
  )
