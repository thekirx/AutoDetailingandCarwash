...
describe('Public branding assets and scope', () => {
  it('ships the approved logo and required web fonts', async () => {
    const assets = [
      ...
      'src/assets/services/ceramic.webp',
  ...
  it('scopes reference alignment to the approved homepage sections', async () => {
    const css = await readFile(projectFile('src/styles.css'), 'utf8')
    const packagesCss = await readFile(projectFile('src/components/public/home/PpfPackagesSection.css'), 'utf8')
    ...
    assert.match(css, /\.ceramic-layout\s*\{[^}]*grid-template-columns:minmax\(260px,\.84fr\) minmax\(0,2\.16fr\)/s)
    assert.match(css, /\.ceramic-intro\s*\{[^}]*container-type:inline-size/s)
    assert.match(css, /\.ceramic-intro h2\s*\{[^}]*font-size:clamp\(3rem,18cqi,6rem\)/s)
    ...
    /* The package section is the step ladder and comparison approved on
    ...
    /* The comparison is permanently visible under the package ladder. */
    assert.match(packages, /className="bd-cmp-heading bd-reveal"/)
    assert.match(packages, /className="bd-cmp" id="ppf-compare"/)
    assert.doesNotMatch(packages, /bd-cmp-toggle|aria-expanded|hidden=\{!compareOpen\}/)
  ...
  it('keeps the hero hierarchy consistent with the legacy reference', async () => {
    ...
    assert.match(css, /\.display-title\s*\{[^}]*font-weight:var\(--font-weight-public-display\)/s)
