...
describe('C1 provision roles + C10 compensation', () => {
  ...
  it('compensation UI reads scalar columns, not a ghost rules json', async () => {
    ...
    assert.match(insights, /collectInChunks/)
    assert.match(insights, /\.order\('id'/)
    ...
    const queueApi = await read('src/queue/queueApi.js')
    ...
    assert.match(adminApi, /\.order\('staff_id'/)
    const finance = await read('src/pages/FinancePage.jsx')
    assert.match(finance, /collectPaged/)
    assert.doesNotMatch(finance, /\.limit\(300\)/)
    const kpi = await read('src/pages/KpiPage.jsx')
    assert.match(kpi, /collectPaged/)
    assert.doesNotMatch(kpi, /\.limit\(500\)/)
    assert.match(crew, /buildCompensationPostPlan/)
    assert.match(crew, /salary_carwash/)
    assert.doesNotMatch(crew, /attendance_status: m\.is_present_today \? 'present'/)
    const pos = await read('src/pages/PosPage.jsx')
    assert.match(pos, /collectPaged/)
    assert.doesNotMatch(pos, /\.limit\(500\)/)
    assert.doesNotMatch(queueApi, /\.limit\(300\)/)
    assert.doesNotMatch(crm, /\.limit\(2500\)/)
    assert.match(finance, /expense_kind/)
  })
})
