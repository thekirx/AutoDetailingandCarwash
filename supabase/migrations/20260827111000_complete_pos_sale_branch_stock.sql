-- Owner revisions P4 follow-up: complete_pos_sale deducts product_branch_stock.
-- Mirrors 20260819133000_pos_handoff_one_sale; only the product stock block changes.
...
begin
  ...
  loop
    ...
    if line->>'item_type' = 'product' then
      ...
      select p.usage_kind into v_usage
      from public.products p
      where p.id = prod_id and p.is_active and not p.is_archived;
      if not found then
        raise exception 'Product % not found or inactive', prod_id;
      end if;

      -- Fail closed for resellable: missing branch row or qty < sold blocks the sale.
      if coalesce(v_usage, 'resellable') = 'resellable' then
        perform set_config('hakum.allow_stock_decrease', '1', true);
        update public.product_branch_stock pbs
        set qty = pbs.qty - qty, updated_at = clock_timestamp()
        where pbs.product_id = prod_id
          and pbs.branch_slug = v_branch
          and pbs.qty >= qty;
        if not found then
          raise exception 'Insufficient branch stock for product %', prod_id;
        end if;
      end if;

      insert into public.product_stock_movements (
        product_id, delta, reason, sale_id, created_by, movement_type, branch_slug
      ) values (
        prod_id, -qty, 'pos_sale', sale_id, caller, 'sale', v_branch
      );
