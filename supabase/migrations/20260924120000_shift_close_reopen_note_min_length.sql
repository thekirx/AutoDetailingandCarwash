-- Align reject/reopen review notes with Finance UI (min 3 characters).
...
begin
  ...
  if v_id is null or v_action not in ('accept', 'reject', 'lock', 'reopen') then
    raise exception 'id and action (accept|reject|lock|reopen) are required';
  end if;
  if v_action = 'reopen' and caller_role is distinct from 'BossMich' then
    raise exception using errcode = '42501',
  ...
  elsif v_action = 'reject' or v_action = 'reopen' then v_next := 'rejected';
  else v_next := 'locked';
  end if;
  ...
  if v_action = 'reopen' and v_status is distinct from 'accepted' then
    raise exception 'Only accepted reports can be reopened';
  end if;
  ...
  values (
    ...
    case when v_action = 'reopen' then 'Reopened end-of-shift close' else 'Reviewed end-of-shift close' end,
